...
execute if score zero.tick server matches 0 as @e[type=armor_stand,tag=mech.give] if score @s mech.power matches 1.. at @s positioned ~ ~1 ~ run function mechanics:conduit/transfer_power

# Turret
execute as @e[type=armor_stand,tag=mech.turret] at @s run tp @s ~ ~ ~ ~.5 ~
